...
OnvifDevice.prototype.fetchSnapshot = function (callback) {
    ...
    // let ourl = mUrl.parse(this.current_profile['snapshot']);
		let ourl = mUrl.parse("http://localhost:3000/images/test.jpg");
		let options = {
...
/* ------------------------------------------------------------------
* Method: ptzMoveAbs(params[, callback])
* - params:
*   - position:
*     - x     | Float   | required |
*     - y     | Float   | required |
...
*     - z     | Float   | required | speed for zoom (in the range of -1.0 to 1.0)
* ---------------------------------------------------------------- */
OnvifDevice.prototype.ptzMoveAbs = function (params, callback) {
	let promise = new Promise((resolve, reject) => {
		if (!this.current_profile) {
			reject(new Error('No media profile is selected.'));
			return;
		...
		let p = {
			...
			'Speed': { 'x': sx, 'y': sy, 'z': sz }
		};
		this.ptz_moving = true;
		this.services['ptz'].absoluteMove(p).then(() => {
			resolve();
		}).catch((error) => {
			reject(error);
		});
